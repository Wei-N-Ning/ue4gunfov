...
void ADebugHud::DrawHUD()
{
	...
	if (bDisplayDebugMessages)
	{
		...
		{
			for (const auto& DebugMessage : DebugMessagesById[Id])
			{
				constexpr float LineHeight = 17.0f;
				constexpr float LineHorizontalMargin = 5.0f;
				const FVector2D Position(LineHorizontalMargin, Y);
				Y += LineHeight * DebugMessage.TextSize;
				FCanvasTextItem TextItem(Position, DebugMessage.Message, DebugFont, DebugMessage.Color);
				TextItem.Scale = FVector2D(DebugMessage.TextSize, DebugMessage.TextSize);
